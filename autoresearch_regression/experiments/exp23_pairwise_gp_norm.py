"""exp23 — Pairwise + gene-packed batching + per-gene loss normalization.

Same as exp22 but the pairwise ranking loss is averaged per gene before
averaging across genes, preventing high-condition-count genes from dominating.
"""
...
def train() -> dict[str, Any]:
    ...
    elog = EpochLog("exp23_pairwise_gp_norm")
    for epoch in range(epochs):
        ...
        for x, y, gid in train_loader:
            ...
            rank_loss = pairwise_ranking_loss(
                pred, y, gid, margin=margin, normalize_per_gene=True,
            )
