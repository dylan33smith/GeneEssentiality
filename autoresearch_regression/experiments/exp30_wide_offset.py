"""exp30 — WideOffsetResidualModel (wider offset head, unchanged gene head).

Tests whether extra capacity in the condition-interaction path improves
within-gene ordering without repeating the exp05 'deeper everywhere' failure.
"""
...
from autoresearch_regression.experiments._shared import WideOffsetResidualModel
...
def train() -> dict[str, Any]:
    ...
    model = WideOffsetResidualModel(GENE_EMBED_DIM, condition_dim).to(device)
    loss_fn = nn.MSELoss()
    optimizer = AdamW(model.parameters(), lr=lr, weight_decay=wd)
    scheduler = ReduceLROnPlateau(optimizer, mode="min", factor=0.5, patience=5)
    early_stop = early_stopper_from_env()

    elog = EpochLog("exp30_wide_offset")
