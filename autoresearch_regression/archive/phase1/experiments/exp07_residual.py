"""exp07 — Residual model: gene_mean subnetwork + condition offset subnetwork.

The idea is to decompose the prediction into a gene-level baseline fitness
(from the ProteomeLM embedding alone) plus an offset that depends on the
experimental condition vector.  This mirrors the biological expectation that
most of the fitness signal is gene-intrinsic, with conditions modulating it.
"""
...
class ResidualModel(nn.Module):
    ...
    def __init__(self, gene_embed_dim: int, condition_dim: int):
        super().__init__()
